...
**ALWAYS read [`project-index.json`](project-index.json) FIRST** at the start of each session. It is the machine-readable map of this repo and is faster + more accurate than `find`/`tree`/`ls`. It contains:

- `subprojects` — annotated map of all subprojects (backend, frontend, the two MCP servers, RAG, the feature-flags layer) with entry points and key paths.
- `system_folders` — `.claude/`, `docs/` (incl. the live `docs/project-data/` RAG corpus), `uploads/` and their purpose.
- `hard_rules` — project-wide rules (mirrored below).
- `ai_routing` — which MCP/tool/folder answers which kind of question.
- `filesystem_tree` — full tree (auto-maintained, depth 4).

For a module deep-dive, read its spec in [`docs/specs/`](docs/specs/). For the system map, see [`docs/architecture/overview.md`](docs/architecture/overview.md).
...
- delete / rename files or folders,
- change a subproject's purpose.

How: run `python3 .claude/scripts/update_project_index.py` (refreshes `filesystem_tree` + `last_updated`; no-ops if nothing structural changed). Or rely on the **PostToolUse hook** in `.claude/settings.local.json`, which runs it automatically after `Write`/`Edit`/`Bash`. `WATCH_PATHS` is tuned to `backend/`, `frontend/src/`, `mcp-servers/`, `rag/`. For the 4-step pattern used to document new modules, see the examples in `docs/specs/`.
...
# Dev: runs backend (nodemon, :5001) and frontend (CRA, :3000) concurrently
npm run dev
...
The root `package.json` is the **backend** project (Express + Mongoose). `frontend/` is a **separate CRA app** with its own `package.json` and lockfile. `npm run dev` uses `concurrently` to run both. The frontend's `package.json` declares `"proxy": "http://127.0.0.1:5001"` (backend `PORT` in `.env`), so frontend code calls `/api/...` directly without CORS config.
