...
> **🔄 TODO(audit-2026-06-07):** This runbook describes a MongoDB `FeatureFlag` collection + `evaluateFlag()` that **do not exist** in this fork. The real implementation is `backend/features.json` mutated **only** through the feature-flags MCP (`set_feature_state` / `adjust_traffic_rollout`) — see `docs/specs/feature-flags-mcp-spec.md` and `docs/adr/adr-006-features-json-shared-store.md`. Rewrite the procedure against the MCP path; re-ingest RAG after.
