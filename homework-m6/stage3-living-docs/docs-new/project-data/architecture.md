...
> **🔄 TODO(audit-2026-06-07):** Two sections diverge from current code — reconcile or re-verify:
> - **Upload auth** — this doc lists `POST /api/upload` as admin-only, but `backend/routes/uploadRoutes.js:37` mounts it with **no `protect, admin`** (Stage 1 finding). The code is the gap; treat the doc as the *intended* contract.
> - **userModel pre-save hook** — this doc claims re-hashing is prevented, but `backend/models/userModel.js:34` is missing `return next()`, so passwords are double-hashed on profile-only updates.
> See `homework-m6/stage1-code-review/synthesis.md`. A RAG re-ingest (`python rag/ingest.py`) is needed for search-docs to surface this marker.
