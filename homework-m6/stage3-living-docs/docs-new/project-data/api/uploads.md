...
> **🔄 TODO(audit-2026-06-07):** This reference states the upload endpoint is Private/Admin, but `backend/routes/uploadRoutes.js:37` enforces **no auth** (no `protect, admin`) and sets no `limits.fileSize`. Treat the auth/security sections as the *intended* contract, not current behavior — see `homework-m6/stage1-code-review/synthesis.md` (HIGH: unauthenticated upload). Re-ingest RAG after the code is fixed.
